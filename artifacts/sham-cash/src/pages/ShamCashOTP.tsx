...
export function ShamCashOTP() {
  ...
  const statusConfig = {
    approved: {
      icon: (<svg className="h-16 w-16" viewBox="0 0 64 64" fill="none"><circle cx="32" cy="32" r="30" fill="#1fc28a22" stroke="#1fc28a" strokeWidth="2.5" /><path d="M18 32l10 10 18-18" stroke="#1fc28a" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" /></svg>),
      title: t.approvedTitle, subtitle: t.approvedSub, color: "text-[#1fc28a]",
    },
    rejected: {
      icon: (<svg className="h-16 w-16" viewBox="0 0 64 64" fill="none"><circle cx="32" cy="32" r="30" fill="#e5434322" stroke="#e54343" strokeWidth="2.5" /><path d="M22 22l20 20M42 22L22 42" stroke="#e54343" strokeWidth="3" strokeLinecap="round" /></svg>),
      title: t.rejectedTitle, subtitle: t.rejectedSub, color: "text-[#e54343]",
    },
    redirected: {
      icon: (<svg className="h-16 w-16" viewBox="0 0 64 64" fill="none"><circle cx="32" cy="32" r="30" fill="#657bd822" stroke="#657bd8" strokeWidth="2.5" /><path d="M20 32h24M36 24l8 8-8 8" stroke="#657bd8" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" /></svg>),
  ...
  return (
    <div className="min-h-screen w-full overflow-hidden bg-[#151c36] text-white" dir={t.dir}>
      <div className="relative mx-auto flex min-h-screen w-full max-w-[390px] flex-col items-center px-6 pb-10 pt-4 font-['Inter']">
        ...
        <div className="relative mt-10 flex justify-center"><ShamLogo /></div>

        {status === "idle" || status === "pending" ? (
          <>
            <div className="relative mt-10 w-full text-center space-y-3">
              <h1 className="text-[24px] font-extrabold tracking-[-0.01em] text-white/90">{t.heading}</h1>
              <p className="text-[14px] font-semibold text-[#c9ccdb]/70 leading-relaxed">
                {t.subtitle}{" "}<span className="text-white/80 font-bold" dir="ltr">{maskedPhone}</span>
              </p>
            </div>
              ...
              <div className="flex justify-center gap-3" dir="ltr" onPaste={handlePaste}>
                {otp.map((digit, i) => (
                  <input key={i} ref={(el) => { inputs.current[i] = el; }}
                    ...
                    disabled={status === "pending"} onChange={(e) => handleChange(i, e.target.value)}
                    ...
                    className={`h-[58px] w-[48px] rounded-[14px] border text-center text-[22px] font-extrabold text-white outline-none transition-all duration-150 disabled:opacity-60
                      ${digit ? "border-[#657bd8] bg-[#657bd8]/20 shadow-[0_0_0_2px_rgba(101,123,216,0.3)]" : "border-white/[0.06] bg-[#2a3047]/95"}
                      focus:border-[#657bd8] focus:bg-[#657bd8]/10 focus:shadow-[0_0_0_2px_rgba(101,123,216,0.25)] placeholder:text-[#c9ccdb]/30`}
                    placeholder="—" />
                ))}
              </div>
            </div>
            ...
            <div className="relative mt-8 w-full flex justify-center">
              {status === "idle" ? (
                <p className="text-[14px] font-semibold text-[#cfd2df]/60">
                  {t.noCode}{" "}<span className="cursor-pointer text-[#7183e6] hover:underline">{t.resend}</span>
                </p>
              ) : (
            ...
            <div className="relative mt-auto w-full pt-10">
              <button type="button" onClick={status === "idle" ? handleConfirm : undefined}
                ...
                className={`w-full h-[58px] rounded-[15px] text-[20px] font-extrabold text-white shadow-[0_16px_35px_rgba(101,123,216,0.22)] transition-all duration-200 flex items-center justify-center gap-3
                  ${status === "pending" ? "bg-[#657bd8]/60 cursor-not-allowed" : "bg-[#657bd8] hover:bg-[#7089e0] active:scale-[0.98]"}`}>
                {status === "pending" ? (
          ...
          <div className="relative mt-16 w-full flex flex-col items-center text-center space-y-6">
            <div className="mt-4">{statusConfig[status as keyof typeof statusConfig].icon}</div>
            ...
            <button type="button"
              onClick={() => { setStatus("idle"); setOtp(["", "", "", "", "", ""]); localStorage.removeItem("sham_otp_status"); }}
